#ifndef FTD20_RSPUSERPASSWORDUPDATE_H
#define FTD20_RSPUSERPASSWORDUPDATE_H
...
struct RspUserPasswordUpdate : public Package
{
	...
	void clear()
	{
        memset(&rspUserPasswordUpdateField, 0, sizeof(RspUserPasswordUpdateField));
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_RspUserPasswordUpdateField)
        {
        	memcpy(&rspUserPasswordUpdateField, &field.rspUserPasswordUpdateField, sizeof(RspUserPasswordUpdateField));
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspUserPasswordUpdate&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspUserPasswordUpdate&  package)
	{
	...
	static void convertToFtdcString(const RspUserPasswordUpdate& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //RspUserPasswordUpdateField
        RspUserPasswordUpdateFieldHelper::writeBuffer(package.rspUserPasswordUpdateField,
        ...
        fieldHeader.fid = FID_RspUserPasswordUpdateField;
