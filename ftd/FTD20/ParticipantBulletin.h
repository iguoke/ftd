...
#ifndef FTD20_PARTICIPANTBULLETIN_H
#define FTD20_PARTICIPANTBULLETIN_H
...
struct ParticipantBulletin : public Package
{
	...
	void clear()
	{
        memset(&bulletinField, 0, sizeof(BulletinField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_BulletinField)
        {
        	memcpy(&bulletinField, &field.bulletinField, sizeof(BulletinField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ParticipantBulletin&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ParticipantBulletin&  package)
	{
	...
	static void convertToFtdcString(const ParticipantBulletin& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //BulletinField
        BulletinFieldHelper::writeBuffer(package.bulletinField,
        ...
        fieldHeader.fid = FID_BulletinField;
