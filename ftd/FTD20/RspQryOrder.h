...
#ifndef FTD20_RSPQRYORDER_H
#define FTD20_RSPQRYORDER_H
...
struct RspQryOrder : public Package
{

    std::vector<OrderStatusField> orderStatusFields;
    ErrorFieldPtr pErrorField;


	void clear()
	{
        orderStatusFields.clear();
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_OrderStatusField)
        {
        	orderStatusFields.push_back(field.orderStatusField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryOrder&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryOrder&  package)
	{
	...
	static void convertToFtdcString(const RspQryOrder& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<OrderStatusField> 
        vecSize = package.orderStatusFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	OrderStatusFieldHelper::writeBuffer(package.orderStatusFields[i],
        	...
        	fieldHeader.fid = FID_OrderStatusField;
