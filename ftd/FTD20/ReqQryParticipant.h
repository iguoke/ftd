#ifndef FTD20_REQQRYPARTICIPANT_H
#define FTD20_REQQRYPARTICIPANT_H
...
struct ReqQryParticipant : public Package
{
	...
	void clear()
	{
        memset(&reqQryParticipantField, 0, sizeof(ReqQryParticipantField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryParticipantField)
        {
        	memcpy(&reqQryParticipantField, &field.reqQryParticipantField, sizeof(ReqQryParticipantField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryParticipant&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryParticipant&  package)
	{
	...
	static void convertToFtdcString(const ReqQryParticipant& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //ReqQryParticipantField
        ReqQryParticipantFieldHelper::writeBuffer(package.reqQryParticipantField,
        ...
        fieldHeader.fid = FID_ReqQryParticipantField;
