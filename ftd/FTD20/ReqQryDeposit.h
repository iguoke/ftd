#ifndef FTD20_REQQRYDEPOSIT_H
#define FTD20_REQQRYDEPOSIT_H
...
struct ReqQryDeposit : public Package
{
	...
	void clear()
	{
        memset(&reqQryDepositField, 0, sizeof(ReqQryDepositField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryDepositField)
        {
        	memcpy(&reqQryDepositField, &field.reqQryDepositField, sizeof(ReqQryDepositField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryDeposit&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryDeposit&  package)
	{
	...
	static void convertToFtdcString(const ReqQryDeposit& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //ReqQryDepositField
        ReqQryDepositFieldHelper::writeBuffer(package.reqQryDepositField,
        ...
        fieldHeader.fid = FID_ReqQryDepositField;
