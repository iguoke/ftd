#ifndef FTD20_REQQRYMARKETORDERDATA_H
#define FTD20_REQQRYMARKETORDERDATA_H
...
struct ReqQryMarketOrderData : public Package
{
	...
	void clear()
	{
        memset(&reqQryMarketOrderDataField, 0, sizeof(ReqQryMarketOrderDataField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryMarketOrderDataField)
        {
        	memcpy(&reqQryMarketOrderDataField, &field.reqQryMarketOrderDataField, sizeof(ReqQryMarketOrderDataField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryMarketOrderData&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryMarketOrderData&  package)
	{
	...
	static void convertToFtdcString(const ReqQryMarketOrderData& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //ReqQryMarketOrderDataField
        ReqQryMarketOrderDataFieldHelper::writeBuffer(package.reqQryMarketOrderDataField,
        ...
        fieldHeader.fid = FID_ReqQryMarketOrderDataField;
