#ifndef FTD20_RSPORDERACTION_H
#define FTD20_RSPORDERACTION_H
...
struct RspOrderAction : public Package
{
	...
	{
        if (fid == FID_OrderActionField)
        {
        	memcpy(&orderActionField, &field.orderActionField, sizeof(OrderActionField));
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspOrderAction&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspOrderAction&  package)
	{
	...
	static void convertToFtdcString(const RspOrderAction& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //OrderActionField
        OrderActionFieldHelper::writeBuffer(package.orderActionField,
        ...
        fieldHeader.fid = FID_OrderActionField;
