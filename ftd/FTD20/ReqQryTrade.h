...
struct ReqQryTrade : public Package
{
	...
	{
        if (fid == FID_ReqQryTradeField)
        {
        	memcpy(&reqQryTradeField, &field.reqQryTradeField, sizeof(ReqQryTradeField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryTrade&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryTrade&  package)
	{
	...
	static void convertToFtdcString(const ReqQryTrade& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //ReqQryTradeField
        ReqQryTradeFieldHelper::writeBuffer(package.reqQryTradeField,
        ...
        fieldHeader.fid = FID_ReqQryTradeField;
