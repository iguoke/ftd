#ifndef FTD20_REQQRYORDER_H
#define FTD20_REQQRYORDER_H
...
struct ReqQryOrder : public Package
{
	...
	{
        if (fid == FID_ReqQryOrderField)
        {
        	memcpy(&reqQryOrderField, &field.reqQryOrderField, sizeof(ReqQryOrderField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryOrder&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryOrder&  package)
	{
	...
	static void convertToFtdcString(const ReqQryOrder& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //ReqQryOrderField
        ReqQryOrderFieldHelper::writeBuffer(package.reqQryOrderField,
        ...
        fieldHeader.fid = FID_ReqQryOrderField;
