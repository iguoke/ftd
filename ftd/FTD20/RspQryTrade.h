...
#ifndef FTD20_RSPQRYTRADE_H
#define FTD20_RSPQRYTRADE_H
...
struct RspQryTrade : public Package
{

    std::vector<TradeInsertSingleField> tradeInsertSingleFields;
    ErrorFieldPtr pErrorField;


	void clear()
	{
        tradeInsertSingleFields.clear();
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_TradeInsertSingleField)
        {
        	tradeInsertSingleFields.push_back(field.tradeInsertSingleField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryTrade&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryTrade&  package)
	{
	...
	static void convertToFtdcString(const RspQryTrade& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<TradeInsertSingleField> 
        vecSize = package.tradeInsertSingleFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	TradeInsertSingleFieldHelper::writeBuffer(package.tradeInsertSingleFields[i],
        	...
        	fieldHeader.fid = FID_TradeInsertSingleField;
