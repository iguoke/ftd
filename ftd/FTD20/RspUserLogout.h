#ifndef FTD20_RSPUSERLOGOUT_H
#define FTD20_RSPUSERLOGOUT_H
...
struct RspUserLogout : public Package
{
	...
	void clear()
	{
        memset(&rspUserLogoutField, 0, sizeof(RspUserLogoutField));
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_RspUserLogoutField)
        {
        	memcpy(&rspUserLogoutField, &field.rspUserLogoutField, sizeof(RspUserLogoutField));
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspUserLogout&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspUserLogout&  package)
	{
	...
	static void convertToFtdcString(const RspUserLogout& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //RspUserLogoutField
        RspUserLogoutFieldHelper::writeBuffer(package.rspUserLogoutField,
        ...
        fieldHeader.fid = FID_RspUserLogoutField;
