#ifndef FTD20_RSPQRYUSER_H
#define FTD20_RSPQRYUSER_H
...
struct RspQryUser : public Package
{

    std::vector<UserField> userFields;
	...
	{
        if (fid == FID_UserField)
        {
        	userFields.push_back(field.userField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryUser&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryUser&  package)
	{
	...
	static void convertToFtdcString(const RspQryUser& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<UserField> 
        vecSize = package.userFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	UserFieldHelper::writeBuffer(package.userFields[i],
