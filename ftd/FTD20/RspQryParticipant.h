...
#ifndef FTD20_RSPQRYPARTICIPANT_H
#define FTD20_RSPQRYPARTICIPANT_H
...
struct RspQryParticipant : public Package
{

    std::vector<ParticipantField> participantFields;
	...
	{
        if (fid == FID_ParticipantField)
        {
        	participantFields.push_back(field.participantField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryParticipant&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryParticipant&  package)
	{
	...
	static void convertToFtdcString(const RspQryParticipant& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<ParticipantField> 
        vecSize = package.participantFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	ParticipantFieldHelper::writeBuffer(package.participantFields[i],
        	...
        	fieldHeader.fid = FID_ParticipantField;
