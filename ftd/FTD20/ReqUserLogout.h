...
#ifndef FTD20_REQUSERLOGOUT_H
#define FTD20_REQUSERLOGOUT_H
...
struct ReqUserLogout : public Package
{
	...
	{
        if (fid == FID_ReqUserLogoutField)
        {
        	memcpy(&reqUserLogoutField, &field.reqUserLogoutField, sizeof(ReqUserLogoutField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqUserLogout&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqUserLogout&  package)
	{
	...
	static void convertToFtdcString(const ReqUserLogout& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //ReqUserLogoutField
        ReqUserLogoutFieldHelper::writeBuffer(package.reqUserLogoutField,
        ...
        fieldHeader.fid = FID_ReqUserLogoutField;
