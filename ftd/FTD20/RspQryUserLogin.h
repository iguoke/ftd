#ifndef FTD20_RSPQRYUSERLOGIN_H
#define FTD20_RSPQRYUSERLOGIN_H
...
struct RspQryUserLogin : public Package
{

    std::vector<UserLoginField> userLoginFields;
    ErrorFieldPtr pErrorField;


	void clear()
	{
        userLoginFields.clear();
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_UserLoginField)
        {
        	userLoginFields.push_back(field.userLoginField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryUserLogin&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryUserLogin&  package)
	{
	...
	static void convertToFtdcString(const RspQryUserLogin& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<UserLoginField> 
        vecSize = package.userLoginFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	UserLoginFieldHelper::writeBuffer(package.userLoginFields[i],
        	...
        	fieldHeader.fid = FID_UserLoginField;
