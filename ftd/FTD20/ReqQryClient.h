#ifndef FTD20_REQQRYCLIENT_H
#define FTD20_REQQRYCLIENT_H
...
struct ReqQryClient : public Package
{
	...
	void clear()
	{
        memset(&reqQryClientField, 0, sizeof(ReqQryClientField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryClientField)
        {
        	memcpy(&reqQryClientField, &field.reqQryClientField, sizeof(ReqQryClientField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryClient&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryClient&  package)
	{
	...
	static void convertToFtdcString(const ReqQryClient& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //ReqQryClientField
        ReqQryClientFieldHelper::writeBuffer(package.reqQryClientField,
        ...
        fieldHeader.fid = FID_ReqQryClientField;
