#ifndef FTD20_RSPQRYPOSITION_H
#define FTD20_RSPQRYPOSITION_H
...
struct RspQryPosition : public Package
{

    std::vector<PositionField> positionFields;
	...
	{
        if (fid == FID_PositionField)
        {
        	positionFields.push_back(field.positionField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryPosition&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryPosition&  package)
	{
	...
	static void convertToFtdcString(const RspQryPosition& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<PositionField> 
        vecSize = package.positionFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	PositionFieldHelper::writeBuffer(package.positionFields[i],
        	...
        	fieldHeader.fid = FID_PositionField;
