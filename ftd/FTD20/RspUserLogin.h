#ifndef FTD20_RSPUSERLOGIN_H
#define FTD20_RSPUSERLOGIN_H
...
struct RspUserLogin : public Package
{
	...
	void clear()
	{
        memset(&rspUserLoginField, 0, sizeof(RspUserLoginField));
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_RspUserLoginField)
        {
        	memcpy(&rspUserLoginField, &field.rspUserLoginField, sizeof(RspUserLoginField));
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspUserLogin&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspUserLogin&  package)
	{
	...
	static void convertToFtdcString(const RspUserLogin& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //RspUserLoginField
        RspUserLoginFieldHelper::writeBuffer(package.rspUserLoginField,
        ...
        fieldHeader.fid = FID_RspUserLoginField;
