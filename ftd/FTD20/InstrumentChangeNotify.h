#ifndef FTD20_INSTRUMENTCHANGENOTIFY_H
#define FTD20_INSTRUMENTCHANGENOTIFY_H
...
struct InstrumentChangeNotify : public Package
{
	...
	bool mergeField(const Field& field, int fid)
	{
        ...
        {
        	memcpy(&instrumentField, &field.instrumentField, sizeof(InstrumentField));
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, InstrumentChangeNotify&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, InstrumentChangeNotify&  package)
	{
	...
	static void convertToFtdcString(const InstrumentChangeNotify& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //InstrumentField
        InstrumentFieldHelper::writeBuffer(package.instrumentField,
