#ifndef FTD20_REQQRYUSERLOGIN_H
#define FTD20_REQQRYUSERLOGIN_H
...
struct ReqQryUserLogin : public Package
{
	...
	void clear()
	{
        memset(&reqQryUserLoginField, 0, sizeof(ReqQryUserLoginField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryUserLoginField)
        {
        	memcpy(&reqQryUserLoginField, &field.reqQryUserLoginField, sizeof(ReqQryUserLoginField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryUserLogin&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryUserLogin&  package)
	{
	...
	static void convertToFtdcString(const ReqQryUserLogin& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //ReqQryUserLoginField
        ReqQryUserLoginFieldHelper::writeBuffer(package.reqQryUserLoginField,
        ...
        fieldHeader.fid = FID_ReqQryUserLoginField;
