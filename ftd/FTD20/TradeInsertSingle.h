#ifndef FTD20_TRADEINSERTSINGLE_H
#define FTD20_TRADEINSERTSINGLE_H
...
struct TradeInsertSingle : public Package
{
	...
	void clear()
	{
        memset(&tradeInsertSingleField, 0, sizeof(TradeInsertSingleField));			
	}
	...
	bool mergeField(const Field& field, int fid)
	{
        ...
        {
        	memcpy(&tradeInsertSingleField, &field.tradeInsertSingleField, sizeof(TradeInsertSingleField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, TradeInsertSingle&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, TradeInsertSingle&  package)
	{
	...
	static void convertToFtdcString(const TradeInsertSingle& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //TradeInsertSingleField
        TradeInsertSingleFieldHelper::writeBuffer(package.tradeInsertSingleField,
