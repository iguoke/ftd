...
#ifndef FTD20_REQQRYPOSITION_H
#define FTD20_REQQRYPOSITION_H
...
struct ReqQryPosition : public Package
{
	...
	void clear()
	{
        memset(&reqQryPositionField, 0, sizeof(ReqQryPositionField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryPositionField)
        {
        	memcpy(&reqQryPositionField, &field.reqQryPositionField, sizeof(ReqQryPositionField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryPosition&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryPosition&  package)
	{
	...
	static void convertToFtdcString(const ReqQryPosition& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //ReqQryPositionField
        ReqQryPositionFieldHelper::writeBuffer(package.reqQryPositionField,
        ...
        fieldHeader.fid = FID_ReqQryPositionField;
