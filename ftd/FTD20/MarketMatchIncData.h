...
#ifndef FTD20_MARKETMATCHINCDATA_H
#define FTD20_MARKETMATCHINCDATA_H
...
struct MarketMatchIncData : public Package
{

    std::vector<MarketMatchIncDataField> marketMatchIncDataFields;
    ErrorFieldPtr pErrorField;


	void clear()
	{
        marketMatchIncDataFields.clear();
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_MarketMatchIncDataField)
        {
        	marketMatchIncDataFields.push_back(field.marketMatchIncDataField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, MarketMatchIncData&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, MarketMatchIncData&  package)
	{
	...
	static void convertToFtdcString(const MarketMatchIncData& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<MarketMatchIncDataField> 
        vecSize = package.marketMatchIncDataFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	MarketMatchIncDataFieldHelper::writeBuffer(package.marketMatchIncDataFields[i],
        	...
        	fieldHeader.fid = FID_MarketMatchIncDataField;
