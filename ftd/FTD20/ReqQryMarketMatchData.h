...
#ifndef FTD20_REQQRYMARKETMATCHDATA_H
#define FTD20_REQQRYMARKETMATCHDATA_H
...
struct ReqQryMarketMatchData : public Package
{
	...
	void clear()
	{
        memset(&reqQryMarketMatchDataField, 0, sizeof(ReqQryMarketMatchDataField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryMarketMatchDataField)
        {
        	memcpy(&reqQryMarketMatchDataField, &field.reqQryMarketMatchDataField, sizeof(ReqQryMarketMatchDataField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryMarketMatchData&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryMarketMatchData&  package)
	{
	...
	static void convertToFtdcString(const ReqQryMarketMatchData& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //ReqQryMarketMatchDataField
        ReqQryMarketMatchDataFieldHelper::writeBuffer(package.reqQryMarketMatchDataField,
        ...
        fieldHeader.fid = FID_ReqQryMarketMatchDataField;
