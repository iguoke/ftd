#ifndef FTD20_REQQRYINSTRUMENT_H
#define FTD20_REQQRYINSTRUMENT_H
...
struct ReqQryInstrument : public Package
{
	...
	void clear()
	{
        memset(&reqQryInstrumentField, 0, sizeof(ReqQryInstrumentField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryInstrumentField)
        {
        	memcpy(&reqQryInstrumentField, &field.reqQryInstrumentField, sizeof(ReqQryInstrumentField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryInstrument&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryInstrument&  package)
	{
	...
	static void convertToFtdcString(const ReqQryInstrument& package, std::vector<std::string>& ftdcMsgs)
	{
        ...
        ReqQryInstrumentFieldHelper::writeBuffer(package.reqQryInstrumentField,
        ...
        fieldHeader.fid = FID_ReqQryInstrumentField;
