#ifndef FTD20_REQQRYINSTRUMENTSTATUS_H
#define FTD20_REQQRYINSTRUMENTSTATUS_H
...
struct ReqQryInstrumentStatus : public Package
{
	...
	void clear()
	{
        memset(&reqQryInstrumentStatusField, 0, sizeof(ReqQryInstrumentStatusField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryInstrumentStatusField)
        {
        	memcpy(&reqQryInstrumentStatusField, &field.reqQryInstrumentStatusField, sizeof(ReqQryInstrumentStatusField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryInstrumentStatus&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryInstrumentStatus&  package)
	{
	...
	static void convertToFtdcString(const ReqQryInstrumentStatus& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //ReqQryInstrumentStatusField
        ReqQryInstrumentStatusFieldHelper::writeBuffer(package.reqQryInstrumentStatusField,
        ...
        fieldHeader.fid = FID_ReqQryInstrumentStatusField;
