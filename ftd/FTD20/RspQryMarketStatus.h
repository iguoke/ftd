#ifndef FTD20_RSPQRYMARKETSTATUS_H
#define FTD20_RSPQRYMARKETSTATUS_H
...
struct RspQryMarketStatus : public Package
{

    std::vector<MarketStatusField> marketStatusFields;
    ErrorFieldPtr pErrorField;


	void clear()
	{
        marketStatusFields.clear();
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_MarketStatusField)
        {
        	marketStatusFields.push_back(field.marketStatusField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryMarketStatus&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryMarketStatus&  package)
	{
	...
	static void convertToFtdcString(const RspQryMarketStatus& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<MarketStatusField> 
        vecSize = package.marketStatusFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	MarketStatusFieldHelper::writeBuffer(package.marketStatusFields[i],
        	...
        	fieldHeader.fid = FID_MarketStatusField;
