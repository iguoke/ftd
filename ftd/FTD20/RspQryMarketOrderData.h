...
#ifndef FTD20_RSPQRYMARKETORDERDATA_H
#define FTD20_RSPQRYMARKETORDERDATA_H
...
struct RspQryMarketOrderData : public Package
{

    std::vector<MarketOrderDataField> marketOrderDataFields;
    ErrorFieldPtr pErrorField;


	void clear()
	{
        marketOrderDataFields.clear();
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_MarketOrderDataField)
        {
        	marketOrderDataFields.push_back(field.marketOrderDataField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryMarketOrderData&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryMarketOrderData&  package)
	{
	...
	static void convertToFtdcString(const RspQryMarketOrderData& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<MarketOrderDataField> 
        vecSize = package.marketOrderDataFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	MarketOrderDataFieldHelper::writeBuffer(package.marketOrderDataFields[i],
        	...
        	fieldHeader.fid = FID_MarketOrderDataField;
