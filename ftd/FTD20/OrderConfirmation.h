#ifndef FTD20_ORDERCONFIRMATION_H
#define FTD20_ORDERCONFIRMATION_H
...
struct OrderConfirmation : public Package
{
	...
	void clear()
	{
        memset(&orderStatusField, 0, sizeof(OrderStatusField));
        pErrorField = nullptr;			
	}
	...
	bool mergeField(const Field& field, int fid)
	{
        ...
        {
        	memcpy(&orderStatusField, &field.orderStatusField, sizeof(OrderStatusField));
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, OrderConfirmation&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, OrderConfirmation&  package)
	{
	...
	static void convertToFtdcString(const OrderConfirmation& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //OrderStatusField
        OrderStatusFieldHelper::writeBuffer(package.orderStatusField,
