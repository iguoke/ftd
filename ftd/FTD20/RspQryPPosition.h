#ifndef FTD20_RSPQRYPPOSITION_H
#define FTD20_RSPQRYPPOSITION_H
...
struct RspQryPPosition : public Package
{

    std::vector<PPositionField> pPositionFields;
    ErrorFieldPtr pErrorField;


	void clear()
	{
        pPositionFields.clear();
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_PPositionField)
        {
        	pPositionFields.push_back(field.pPositionField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryPPosition&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryPPosition&  package)
	{
	...
	static void convertToFtdcString(const RspQryPPosition& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<PPositionField> 
        vecSize = package.pPositionFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	PPositionFieldHelper::writeBuffer(package.pPositionFields[i],
        	...
        	fieldHeader.fid = FID_PPositionField;
