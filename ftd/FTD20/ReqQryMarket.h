#ifndef FTD20_REQQRYMARKET_H
#define FTD20_REQQRYMARKET_H
...
struct ReqQryMarket : public Package
{
	...
	void clear()
	{
        memset(&reqQryMarketField, 0, sizeof(ReqQryMarketField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryMarketField)
        {
        	memcpy(&reqQryMarketField, &field.reqQryMarketField, sizeof(ReqQryMarketField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryMarket&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryMarket&  package)
	{
	...
	static void convertToFtdcString(const ReqQryMarket& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //ReqQryMarketField
        ReqQryMarketFieldHelper::writeBuffer(package.reqQryMarketField,
        ...
        fieldHeader.fid = FID_ReqQryMarketField;
