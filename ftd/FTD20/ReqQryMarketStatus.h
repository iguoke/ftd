...
#ifndef FTD20_REQQRYMARKETSTATUS_H
#define FTD20_REQQRYMARKETSTATUS_H
...
struct ReqQryMarketStatus : public Package
{
	...
	void clear()
	{
        memset(&reqQryMarketStatusField, 0, sizeof(ReqQryMarketStatusField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryMarketStatusField)
        {
        	memcpy(&reqQryMarketStatusField, &field.reqQryMarketStatusField, sizeof(ReqQryMarketStatusField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryMarketStatus&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryMarketStatus&  package)
	{
	...
	static void convertToFtdcString(const ReqQryMarketStatus& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //ReqQryMarketStatusField
        ReqQryMarketStatusFieldHelper::writeBuffer(package.reqQryMarketStatusField,
        ...
        fieldHeader.fid = FID_ReqQryMarketStatusField;
