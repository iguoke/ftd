#ifndef FTD20_MARKETMATCHDATA_H
#define FTD20_MARKETMATCHDATA_H
...
struct MarketMatchData : public Package
{

    std::vector<MarketMatchDataField> marketMatchDataFields;
    std::vector<MarketMatchDataChgField> marketMatchDataChgFields;


	void clear()
	{
        marketMatchDataFields.clear();
        marketMatchDataChgFields.clear();			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_MarketMatchDataField)
        {
        	marketMatchDataFields.push_back(field.marketMatchDataField);
        	return true;
        }
        if (fid == FID_MarketMatchDataChgField)
        {
        	marketMatchDataChgFields.push_back(field.marketMatchDataChgField);
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, MarketMatchData&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, MarketMatchData&  package)
	{
	...
	static void convertToFtdcString(const MarketMatchData& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<MarketMatchDataField> 
        vecSize = package.marketMatchDataFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	MarketMatchDataFieldHelper::writeBuffer(package.marketMatchDataFields[i],
        	...
        	fieldHeader.fid = FID_MarketMatchDataField;
        ...
        //std::vector<MarketMatchDataChgField> 
        vecSize = package.marketMatchDataChgFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	MarketMatchDataChgFieldHelper::writeBuffer(package.marketMatchDataChgFields[i],
        		fieldBuffer, fieldLen);
        	if (MAX_FTDC_LENGTH - (nextWrite - ftdcBuffer) < FTDC_FIELD_HEADER_LENGTH + fieldLen)
        	{					
        		header.contentLength = nextWrite - ftdcBuffer;
        	...
        	FTDCFieldHeader fieldHeader;
        	fieldHeader.fid = FID_MarketMatchDataChgField;
        	fieldHeader.fidLength = fieldLen;
        	nextWrite = writeFTDCFieldHeader(fieldHeader, nextWrite);
        	memcpy(nextWrite, fieldBuffer, fieldLen);
        	nextWrite += fieldLen;
