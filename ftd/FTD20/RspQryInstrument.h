#ifndef FTD20_RSPQRYINSTRUMENT_H
#define FTD20_RSPQRYINSTRUMENT_H
...
struct RspQryInstrument : public Package
{

    std::vector<InstrumentField> instrumentFields;
    ErrorFieldPtr pErrorField;


	void clear()
	{
        instrumentFields.clear();
        pErrorField = nullptr;			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_InstrumentField)
        {
        	instrumentFields.push_back(field.instrumentField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryInstrument&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryInstrument&  package)
	{
	...
	static void convertToFtdcString(const RspQryInstrument& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<InstrumentField> 
        vecSize = package.instrumentFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	InstrumentFieldHelper::writeBuffer(package.instrumentFields[i],
        	...
        	fieldHeader.fid = FID_InstrumentField;
