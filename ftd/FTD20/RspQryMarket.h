...
#ifndef FTD20_RSPQRYMARKET_H
#define FTD20_RSPQRYMARKET_H
...
struct RspQryMarket : public Package
{

    std::vector<MarketField> marketFields;
	...
	{
        if (fid == FID_MarketField)
        {
        	marketFields.push_back(field.marketField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryMarket&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryMarket&  package)
	{
	...
	static void convertToFtdcString(const RspQryMarket& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<MarketField> 
        vecSize = package.marketFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	MarketFieldHelper::writeBuffer(package.marketFields[i],
        	...
        	fieldHeader.fid = FID_MarketField;
