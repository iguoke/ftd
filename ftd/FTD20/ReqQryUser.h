#ifndef FTD20_REQQRYUSER_H
#define FTD20_REQQRYUSER_H
...
struct ReqQryUser : public Package
{
	...
	void clear()
	{
        memset(&reqQryUserField, 0, sizeof(ReqQryUserField));			
	}

	bool mergeField(const Field& field, int fid)
	{
        if (fid == FID_ReqQryUserField)
        {
        	memcpy(&reqQryUserField, &field.reqQryUserField, sizeof(ReqQryUserField));
        	return true;
        }
		return false;
	}

	static bool onFtdcMessage(const std::string& ftdcMsg, ReqQryUser&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, ReqQryUser&  package)
	{
	...
	static void convertToFtdcString(const ReqQryUser& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int fieldLen = 0;
        //ReqQryUserField
        ReqQryUserFieldHelper::writeBuffer(package.reqQryUserField,
        ...
        fieldHeader.fid = FID_ReqQryUserField;
