...
#ifndef FTD20_RSPQRYDEPOSIT_H
#define FTD20_RSPQRYDEPOSIT_H
...
struct RspQryDeposit : public Package
{

    std::vector<DepositField> depositFields;
	...
	{
        if (fid == FID_DepositField)
        {
        	depositFields.push_back(field.depositField);
        	return true;
        }
	...
	static bool onFtdcMessage(const std::string& ftdcMsg, RspQryDeposit&  package)
	{
		FTDCHeader header;
	...
	static bool onFtdcMessage(const FTDCHeader& header, const std::string& ftdcContent, RspQryDeposit&  package)
	{
	...
	static void convertToFtdcString(const RspQryDeposit& package, std::vector<std::string>& ftdcMsgs)
	{
		...
		int vecSize = 0;
		int fieldLen = 0;
        //std::vector<DepositField> 
        vecSize = package.depositFields.size();			
        for (int i = 0; i < vecSize; i++)
        {
        	DepositFieldHelper::writeBuffer(package.depositFields[i],
        	...
        	fieldHeader.fid = FID_DepositField;
