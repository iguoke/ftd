...
#ifndef FTD_SESSIONSTATE_H
#define FTD_SESSIONSTATE_H
...
class SessionState : public PackageStore, public Log
{
...
: m_enabled( true ), m_receivedLogon( false ),
  ...
  PackageStore* store() { return m_pStore; }
  void store(PackageStore* pValue ) { m_pStore = pValue; }
  Log* log() { return m_pLog ? m_pLog : &m_nullLog; }
  void log( Log* pValue ) { m_pLog = pValue; }

  void heartBtInt( const int& value )
  { m_heartBtInt = value; }
  int& heartBtInt()
  { return m_heartBtInt; }
  const int& heartBtInt() const
  { return m_heartBtInt; }
  ...
  void queue( int msgSeqNum, const std::string& message )
  { Locker l( m_mutex ); m_queue[ msgSeqNum ] = message; }
  bool retrieve( int msgSeqNum, std::string& message )
  {
    Locker l( m_mutex );
